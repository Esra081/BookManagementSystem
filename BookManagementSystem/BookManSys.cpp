...
void student_record() {
    ...
    if (usernameTaken) {
        ...
        ofstream outputFile("user_accounts.txt", ios::app); //opens in append mod
    ...
    case 1:
        while (true) {
            ...
            if (inputFile) {
                ...
                inputFile.clear(); /* Clears the input file stream's error state and allows the read position to be set to the beginning of the file.
                seekg function is used to move the read position to a specific location in the file.
                The first parameter specifies the position to move to, and ios::beg indicates the beginning point. */

                inputFile.seekg(0, ios::beg); /* Sets the read position of the file stream to the beginning point,
                enabling the file to be read again from the start. In this case, if the user made an incorrect input,
                the read position may have reached the end of the file. Therefore, we set the read position to the beginning
                to be able to read the file again from the start. */

                // These statements allow the file to be read from the beginning in case the user makes an incorrect input.
